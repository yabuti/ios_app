...
class LoginBloc extends Bloc<LoginEvent, LoginStateModel> {
  ...
  set userInformation(UserResponseModel? value) {
    _userResponse = value;
  }
  ...
  Future<void> _loginEvent(
    ...
    if (success != null) {
      ...
      log('✅ Login successful - User: ${success.user.name}, Token: ${success.accessToken.substring(0, 20)}...', name: 'LoginBloc');
      
      // ALWAYS save token and user data (required for API calls)
      await _saveCredentialDirect(success);
  ...
  /// Direct save credentials (called immediately after login)
  Future<void> _saveCredentialDirect(UserResponseModel userResponse) async {
    try {
      final pref = await SharedPreferences.getInstance();
      await pref.setString('accessToken', userResponse.accessToken);
      await pref.setString('tokenType', userResponse.tokenType);
      await pref.setInt('expiresIn', userResponse.expiresIn);
      await pref.setString('userType', userResponse.userType);
      await pref.setInt('userId', userResponse.user.id);
      await pref.setString('userName', userResponse.user.name);
      await pref.setString('phone', userResponse.user.phone);
      await pref.setString('email', userResponse.user.email);
      await pref.setString('username', userResponse.user.username);
      log('✅ User credentials saved to SharedPreferences', name: 'LoginBloc');
    } catch (e) {
      log('❌ Error saving credentials: $e', name: 'LoginBloc');
    }
  }
