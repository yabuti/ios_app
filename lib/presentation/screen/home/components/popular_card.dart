...
class _PopularCarCardState extends State<PopularCarCard> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<WishlistCubit, WishlistStateModel>(
      listener: (context, state) {
        final wishlistState = state.wishlistState;
        if (wishlistState is WishListAddedLoaded) {
          Utils.successSnackBar(context, wishlistState.message);
        } else if (wishlistState is WishListRemoveLoaded) {
          Utils.successSnackBar(context, wishlistState.message);
        } else if (wishlistState is WishlistStateError) {
          if (wishlistState.statusCode != 401) {
            Utils.failureSnackBar(context, wishlistState.message);
          }
        }
      },
