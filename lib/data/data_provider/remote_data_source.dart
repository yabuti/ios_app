...
class RemoteDataSourcesImpl extends RemoteDataSources {
  ...
  Future addWishList(Uri url) async {
    try {
      ...
      final clientMethod = Future.value(response);
      final responseJsonBody =
          await NetworkParser.callClientWithCatchException(() => clientMethod);
      return responseJsonBody['message'] as String;
    } catch (e) {
  ...
  @override
  Future updateProfile(User body, Uri url) async {
    final request = http.MultipartRequest('POST', url);

    // Add authentication headers
    request.headers.addAll(await _authMultipartHeaders());

    // Add basic fields (not image fields)
    request.fields['name'] = body.name;
    request.fields['email'] = body.email;
    request.fields['phone'] = body.phone;
    request.fields['designation'] = body.designation;
    request.fields['address'] = body.address;

    // Only add image if it's a new local file (not a URL)
    if (body.tempImage.isNotEmpty && !body.tempImage.startsWith('http')) {
      final file = await http.MultipartFile.fromPath('image', body.tempImage);
      request.files.add(file);
    }

    // Only add banner image if it's a new local file (not a URL)
    if (body.bannerTempImage.isNotEmpty && !body.bannerTempImage.startsWith('http')) {
      final bannerFile = await http.MultipartFile.fromPath('banner_image', body.bannerTempImage);
    ...
    // Return the full response body (contains both 'message' and 'user')
    return responseJsonBody;
  }
